...
class Account < ApplicationRecord
  ...
  validate :note_has_sixteen_newlines?, if: -> { local? && will_save_change_to_note? }
  ...
  def note_has_sixteen_newlines?
    errors.add(:note, 'Bio can\'t have more then 16 newlines') unless note.count("\n") <= 16
  end
